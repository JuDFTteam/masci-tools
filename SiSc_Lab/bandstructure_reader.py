...
time_realplot = time.time()
print(time_realplot-time_read)
#%%
# =============================================================================
...
def new_plotfunction_weights(bands, characters, groups, spin, unfolding_weight):
    ...
    for n in bands:
        #a=1
        #weight_k_n[n]
        #k_aux = np.append(k_aux, k_dist)
        #E_aux = np.append(E_aux, E_i(n, spin))
        #w_aux = np.append(w_aux, weight_k_n[n])
        ax1.scatter(k_dist, E_i(n, spin), marker='o', c='b', s = 2 * weight_k_n[n], lw=0)
    time_plot = time.time()
    #ax1.scatter(k_aux, E_aux, marker='o', c='b', s = 2 * w_aux, lw=0)
    plt.xticks(k_special_pt, special_points_label)
    time_done = time.time()
    print("preprocess:")
    print(time_reshape-time_preprocess)
    print("reshape:")
    print(time_plot-time_reshape)
    print("plot:")
    print(time_done-time_plot)
    """
    for i in range(len(special_points)):
...
new_plotfunction_weights(all_bands, [1], all_groups, spin = 0, unfolding_weight = False)
...
time_plotall = time.time()
print(time_plotall-time_realplot)
#%%
