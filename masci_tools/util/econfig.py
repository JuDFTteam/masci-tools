...
def get_econfig(element: Union[str, int], full: bool = False) -> str:
    """
    ...
    if isinstance(element, int):
        econ: str = PERIODIC_TABLE_ELEMENTS.get(element, {}).get('econfig')  #type:ignore
    ...
    if econ is None:
        raise ValueError(f'No electronic configuartion found for {element}')
...
def get_coreconfig(element: Union[str, int], full: bool = False) -> str:
    """
    returns the econfiguration as a string of an element.
...
def rek_econ(econfigstr: str) -> str:
    """
