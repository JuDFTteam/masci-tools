...
imposed alongside those in the introduction where that the solution

\begin{enumerate}
\item relies solely on Open-Source-Software (OSS) with licensing suitable for
    academic use, has a stable release cycle, developer base and documentation,
\item provides graphical control elements (widgets) that interact with plotting
    libraries,
\item ideally works like any web service, i.e. only a modern web browser is
    required to use it,
\item requires only Python and no Web Development knowledge like e.g. Javascript,
    with respect to the product stakeholders.    
\end{enumerate}
...
partially fulfilled the last requirement, so they were discarded.

Jupyter on the other hand only partially fulfills the third requirement: a
Jupyter app cannot, by itself, be used outside a live Jupyter environment
